...
if __name__ == "__main__":
    # Example usage - Enhanced AppDaemon Parser Demo
    parser = AppDaemonParser()
    ...
    sample_files = [
        "../apps/climate.py",
        "../apps/entrance.py",
        "../apps/motion_sensors.py",
        "../apps/telegram_client.py",
    ]
    ...
    for file_path in sample_files:
        try:
            ...
            print(f"Constants used: {sorted(result.constants_used)}")
            print(f"Complexity Score: {result.complexity_score}")

            # File-level aggregated data
            if result.all_mqtt_topics:
                print(f"MQTT Topics: {result.all_mqtt_topics}")
            if result.all_entities:
                print(f"Entities: {result.all_entities[:5]}{'...' if len(result.all_entities) > 5 else ''}")
            if result.all_service_calls:
                print(f"Service Calls: {result.all_service_calls}")
            ...
            for class_info in result.classes:
                ...
                if class_info.state_listeners:
                    print(f"  State Listeners ({len(class_info.state_listeners)}):")
                    for listener in class_info.state_listeners:
                        print(f"    - {listener.callback_method} -> {listener.entity}")

                # MQTT Listeners
                if class_info.mqtt_listeners:
                    print(f"  MQTT Listeners ({len(class_info.mqtt_listeners)}):")
                    for mqtt in class_info.mqtt_listeners:
                        print(f"    - {mqtt.callback_method} -> {mqtt.topic} (namespace: {mqtt.namespace})")

                # Service Calls
                if class_info.service_calls:
                    print(f"  Service Calls ({len(class_info.service_calls)}):")
                    for service in class_info.service_calls[:3]:  # Show first 3
                        print(f"    - {service.service_domain}.{service.service_name} in {service.method_name}")
                    if len(class_info.service_calls) > 3:
                        print(f"    ... and {len(class_info.service_calls) - 3} more")

                # Time Schedules
                if class_info.time_schedules:
                    print(f"  Time Schedules ({len(class_info.time_schedules)}):")
                    for schedule in class_info.time_schedules:
                        print(f"    - {schedule.schedule_type}: {schedule.callback_method}")

                # Device Relationships
                if class_info.device_relationships:
                    print(f"  Device Relationships ({len(class_info.device_relationships)}):")
                    for rel in class_info.device_relationships[:3]:  # Show first 3
                        print(f"    - {rel.trigger_entity} -> {rel.target_entity} ({rel.relationship_type})")

                # Automation Flows
                if class_info.automation_flows:
                    print(f"  Automation Flows ({len(class_info.automation_flows)}):")
                    for flow in class_info.automation_flows[:2]:  # Show first 2
                        print(
                            f"    - {flow.flow_type} in {flow.method_name}: {len(flow.conditions)} conditions, {len(flow.actions)} actions"
                        )
